<?php
    $title = 'Ajout total';
    $cssFile = 'ajout-total.css';
    $jsFile = 'ajout-total-js.php';
	$errors = 'error_handler.js';
?>

<?php ob_start(); ?>

<h1>Ajouter votre équipement</h1>

<div id="total">
    <div class="slideAjout">

        <h2>Ajouter une maison</h2>

        <div class="content">
            <input type="text" name="nom" placeholder="Nom" id="name"><br>
            <input type="text" name="adresse" placeholder="Adresse" id="adresse"><br>
            <input type="text" name="superficie" placeholder="Superficie en m²" id="superficie"><br>

			<br />
			<label id="error_message" style="visibility: hidden; color: red;">
			</label>

            <div class="submit">
                <button type="button" class="bubbly-button" onclick="addItemMaison()">Ajouter</button>
            </div>

            <div id="resume" class="resume"></div>

            <div id="dynamic-list" class="dynamic-list"></div>

        </div>

        <div class="continue">
            <a onclick="plusDivs(1)" class="hvr-wobble-horizontal arrow">
                <img src="data:image/svg+xml;utf8;base64," />
            </a>
        </div>

    </div>

    <div class="slideAjout">

        <h2>Ajout d'un utilisateur</h2>
        <div class="content">
            <!-- <div id="image">
                <img src="public/assets/avatar.png" width="40px" height="40px" alt="icon" id="icon">
            </div> -->
            <div id="informations">
                <input type="text" name="nom" placeholder="Nom" id="nom"><br>
                <input type="text" name="prenom" placeholder="Prénom" id="prenom"><br>
                <input type="text" name="email" placeholder="Adresse mail" id="email"><br>
                <input type="password" name="password" placeholder="Mot de passe" id="mdp"><br>
                <select id="statut" placeholder="Statut" id="statut">
                    <option value="parent">Parent</option>
                    <option value="enfant">Enfant</option>
                    <option value="admin">Administrateur</option>
                </select> <br>
            </div>
        </div>
        <!-- <div id="bouton-avatar">
            <button type="button" class="avatar-button">Ajouter un avatar</button>
        </div> -->
        <div class="submit">
            <button type="button" class="bubbly-button" onclick="addItemUtilisateur()">Ajouter</button>
        </div>

        <div id="resume2" class="resume"></div>

        <div id="dynamic-list2" class="dynamic-list"></div>

        <div class="continue">
            <a onclick="plusDivs(-1)" class="hvr-wobble-horizontal-i arrow">
                <img src="data:image/svg+xml;utf8;base64," />
            </a>
            <a onclick="plusDivs(1)" class="hvr-wobble-horizontal arrow">
                <img src="data:image/svg+xml;utf8;base64," />
            </a>
        </div>

    </div>

    <div class="slideAjout">

        <h2>Ajout d'un capteur</h2>

        <div class="content">

            <select id="sensorTypeSelect" name="sensorTypeSelect">
                <?php foreach ($capteurDispo as $capteur): ?>
                    <option value=" <?= $capteur['nom'] ?>">Capteur de <?= $capteur['nom'] ?></option>
                <?php endforeach; ?>
            </select>

            <select id="roomSelect" name="roomSelect">
                <option value="salon">Salon</option>
                <option value="chambre">Chambre</option>
            </select>

        </div>

        <div class="submit">
            <button type="button" class="bubbly-button" onclick="addItemCapteur()">Ajouter</button>
        </div>

        <div id="resume3" class="resume"></div>

        <div id="dynamic-list3" class="dynamic-list"></div>

        <div class="continue">
            <a onclick="plusDivs(-1)" class="hvr-wobble-horizontal-i arrow">
                <img src="data:image/svg+xml;utf8;base64," />
            </a>
            <a onclick="finalise()" class="hvr-wobble-horizontal arrow">
                <img src="data:image/svg+xml;utf8;base64," />
            </a>
        </div>
    </div>

    <div class="slideAjout">

        <h2>Résumé</h2>


        <div id="final-resume" class="dynamic-list"></div>


        <div class="continue">
            <a onclick="plusDivs(-1);" class="hvr-wobble-horizontal-i arrow">
                <img src="data:image/svg+xml;utf8;base64," />
            </a>
            <form action="index.php?action=addTotal" method="post" id="final-form">
                <input type="hidden" id="input_utilisateur" name="input_utilisateur">
                <input type="hidden" id="input_capteur" name="input_capteur">
                <input type="submit" class="bubbly-button arrow" value="Confirmer">
            </form>

        </div>

    </div>

</div>


<?php $content = ob_get_clean(); ?>

<?php require('template.php') ?>
